<?php
	...
	if (isset($_POST['submit1'])){
		...
		$extension = end($array);
		$path = 'photo/'.$_POST['username']."-".date('YmdHis')."."."$extension";
		copy($_FILES['foto']['tmp_name'],$path);
		$_SESSION['foto']  = $_POST['username']."-".date('YmdHis')."."."$extension";
...
<body>
	...
	<?php
		...
		if ($_SESSION['lives'] == 0){
			...
			$db = mysqli_connect('localhost', 'root', '', 'crazy');
			$query = "INSERT INTO scores (username, score, playtime, foto) VALUES ('".$_COOKIE['username']."', ".$_SESSION['score'].", '".date('Y-m-d H:i:s')."','".$_SESSION['foto']."')";
